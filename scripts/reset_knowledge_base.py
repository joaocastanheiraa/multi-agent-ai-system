...
def truncate_knowledge_base(agent_id=None):
    ...
    try:
        if agent_id:
            ...
            response = client.table("knowledge_base").delete().eq("metadata->>agent_id", agent_id).execute()
...
def reset_and_reload(knowledge_dir, max_tokens=500, agent=None):
    ...
    if not truncate_knowledge_base(agent_id=DOMAIN_TO_AGENT.get(agent) if agent else None):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reset e recarga da base de conhecimento")
    parser.add_argument(
        "--knowledge_dir", 
    ...
    parser.add_argument(
        "--agent", 
        choices=['neurohook', 'retention', 'pain', 'paradigm', 'metaphor', 'conversion'],
