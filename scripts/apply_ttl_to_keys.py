...
def apply_ttl_to_keys(prefix, ttl_seconds=86400, dry_run=True):
    ...
    for key in keys:
        ...
        if current_ttl == -1:
            ...
            logger.info(f"{'[DRY RUN] Aplicaria' if dry_run else 'Aplicado'} TTL de {ttl_seconds}s para chave: {key}")
            updated += 1
    
    logger.info(f"{'[DRY RUN] ' if dry_run else ''}Total de {updated} chaves atualizadas")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prefix", 
        default="chat:",
        help="Prefixo das chaves para aplicar TTL"
    )
    parser.add_argument(
        "--ttl", 
        type=int, 
