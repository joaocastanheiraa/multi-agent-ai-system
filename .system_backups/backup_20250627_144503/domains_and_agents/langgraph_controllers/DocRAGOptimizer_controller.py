...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are DocRAGOptimizer, a specialized AI agent.

Domain: knowledge
Specialization: DocRAGOptimizer operations and management
Controller: DocRAGOptimizer_controller.py

You are an expert in knowledge domain with deep knowledge of DocRAGOptimizer workflows.


## FERRAMENTAS DISPONÍVEIS
Você tem acesso às seguintes ferramentas especializadas:
- **document_processor**: Tool document_processor para DocRAGOptimizer (✅ ATIVA)
- **embedding_generator**: Tool embedding_generator para DocRAGOptimizer (✅ ATIVA)
- **rag_search**: Tool rag_search para DocRAGOptimizer (✅ ATIVA)
...
class FunctionalDocragoptimizerController:
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é DOCRAGOPTIMIZER, um especialista em gestão de conhecimento.
Sua função é organizar, processar e fornecer informações relevantes.
Ajude com pesquisa, documentação e estruturação de conhecimento."""
...
def execute_docragoptimizer_node(state: DocRAGOptimizerState) -> DocRAGOptimizerState:
    """Execute main processing for DocRAGOptimizer"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "DocRAGOptimizer".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de DocRAGOptimizer concluído com sucesso")],
                'agent_name': "DocRAGOptimizer",
                'current_step': 'completed'
            }
        ...
        return {
            **state,
            "error_state": f"Error in DocRAGOptimizer node: {str(e)}"
        }

# Create the StateGraph
workflow_graph = StateGraph(DocRAGOptimizerState)

# Add nodes
workflow_graph.add_node("main_processing", execute_docragoptimizer_node)
...
DocRAGOptimizer_graph = workflow_graph.compile()

def run_docragoptimizer_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the DocRAGOptimizer workflow via LangGraph"""
    initial_state = {
        "messages": messages,
    ...
    final_state = DocRAGOptimizer_graph.invoke(initial_state)
    return final_state
