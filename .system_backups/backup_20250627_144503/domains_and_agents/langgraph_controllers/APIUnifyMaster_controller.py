...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are APIUnifyMaster, a specialized AI agent.

Domain: apis
Specialization: APIUnifyMaster operations and management
Controller: APIUnifyMaster_controller.py

You are an expert in apis domain with deep knowledge of APIUnifyMaster workflows.


## FERRAMENTAS DISPONÍVEIS
Você tem acesso às seguintes ferramentas especializadas:
- **api_client**: Tool api_client para APIUnifyMaster (✅ ATIVA)
- **json_parser**: Tool json_parser para APIUnifyMaster (✅ ATIVA)
- **webhook_handler**: Tool webhook_handler para APIUnifyMaster (✅ ATIVA)
...
#!/usr/bin/env python3
"""
🤖 APIUNIFYMASTER - CONTROLLER FUNCIONAL
...
class FunctionalApiunifymasterController:
    """Controller funcional do apiunifymaster"""
    
    def __init__(self):
        self.agent_name = "apiunifymaster"
        self.domain = "apis"
        self.setup_llm()
        self.load_prompt()
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é APIUNIFYMASTER, um especialista em integração de APIs.
...
# Instância global
functional_apiunifymaster = FunctionalApiunifymasterController()

def run_apiunifymaster(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Função principal de execução"""
    return functional_apiunifymaster.execute(messages)
...
def execute_apiunifymaster_node(state: APIUnifyMasterState) -> APIUnifyMasterState:
    """Execute main processing for APIUnifyMaster"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "APIUnifyMaster".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de APIUnifyMaster concluído com sucesso")],
        ...
        return {
            **state,
            "error_state": f"Error in APIUnifyMaster node: {str(e)}"
        }
...
# Add nodes
workflow_graph.add_node("main_processing", execute_apiunifymaster_node)
...
def run_apiunifymaster_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the APIUnifyMaster workflow via LangGraph"""
    initial_state = {
        "messages": messages,
...
if __name__ == "__main__":
    # Teste do controller
    print(f"🤖 TESTANDO {functional_apiunifymaster.agent_name.upper()} FUNCIONAL")
    print("=" * 50)
    
    test_messages = [HumanMessage(content="Teste de funcionamento do agente apiunifymaster")]
