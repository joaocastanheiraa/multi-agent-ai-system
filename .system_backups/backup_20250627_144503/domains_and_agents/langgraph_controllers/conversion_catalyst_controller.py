...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are conversion_catalyst, a specialized AI agent.
...
- **ab_tester**: Tool ab_tester para conversion_catalyst (✅ ATIVA)
- **conversion_optimizer**: Tool conversion_optimizer para conversion_catalyst (✅ ATIVA)
- **cta_generator**: Tool cta_generator para conversion_catalyst (✅ ATIVA)
- **knowledge_base**: Tool knowledge_base para conversion_catalyst (✅ ATIVA)
- **text_analysis**: Tool text_analysis para conversion_catalyst (✅ ATIVA)
- **web_search**: Tool web_search para conversion_catalyst (✅ ATIVA)
...
- **value_amplifier**: Sub-agente especializado
- **risk_neutralizer**: Sub-agente especializado
- **decision-mapper**: Sub-agente especializado
- **urgency_architect**: Sub-agente especializado
- **command-architect**: Sub-agente especializado
...
class FunctionalConversionCatalystController:
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é CONVERSION_CATALYST, um especialista em copywriting e persuasão.
...
def execute_conversion_catalyst_node(state: conversioncatalystState) -> conversioncatalystState:
    """Execute main processing for conversion catalyst"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "conversion catalyst".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de conversion catalyst concluído com sucesso")],
                'agent_name': "conversion catalyst",
                'current_step': 'completed'
            }
        ...
        return {
            **state,
            "error_state": f"Error in conversion catalyst node: {str(e)}"
        }
...
# Add nodes
workflow_graph.add_node("main_processing", execute_conversion_catalyst_node)
...
def run_conversion_catalyst_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the conversion catalyst workflow via LangGraph"""
    initial_state = {
        "messages": messages,
