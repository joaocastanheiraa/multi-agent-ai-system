...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are PerfectpayAPIMaster, a specialized AI agent.
...
#!/usr/bin/env python3
"""
🤖 PERFECTPAYAPIMASTER - CONTROLLER FUNCIONAL
...
class FunctionalPerfectpayapimasterController:
    """Controller funcional do perfectpayapimaster"""
    
    def __init__(self):
        self.agent_name = "perfectpayapimaster"
        self.domain = "apis"
        self.setup_llm()
        self.load_prompt()
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é PERFECTPAYAPIMASTER, um especialista em integração de APIs.
...
# Instância global
functional_perfectpayapimaster = FunctionalPerfectpayapimasterController()

def run_perfectpayapimaster(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Função principal de execução"""
    return functional_perfectpayapimaster.execute(messages)
...
def execute_perfectpayapimaster_node(state: PerfectpayAPIMasterState) -> PerfectpayAPIMasterState:
    """Execute main processing for PerfectpayAPIMaster"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "PerfectpayAPIMaster".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de PerfectpayAPIMaster concluído com sucesso")],
                'agent_name': "PerfectpayAPIMaster",
                'current_step': 'completed'
            }
        ...
        return {
            **state,
            "error_state": f"Error in PerfectpayAPIMaster node: {str(e)}"
        }

# Create the StateGraph
workflow_graph = StateGraph(PerfectpayAPIMasterState)

# Add nodes
workflow_graph.add_node("main_processing", execute_perfectpayapimaster_node)
...
PerfectpayAPIMaster_graph = workflow_graph.compile()

def run_perfectpayapimaster_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the PerfectpayAPIMaster workflow via LangGraph"""
    initial_state = {
        "messages": messages,
    ...
    final_state = PerfectpayAPIMaster_graph.invoke(initial_state)
    return final_state


if __name__ == "__main__":
    # Teste do controller
    print(f"🤖 TESTANDO {functional_perfectpayapimaster.agent_name.upper()} FUNCIONAL")
    print("=" * 50)
    
    test_messages = [HumanMessage(content="Teste de funcionamento do agente perfectpayapimaster")]
    result = run_perfectpayapimaster(test_messages)
