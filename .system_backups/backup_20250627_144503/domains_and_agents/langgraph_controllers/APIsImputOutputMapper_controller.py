...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are APIsImputOutputMapper, a specialized AI agent.
...
def execute_apisimputoutputmapper_node(state: APIsImputOutputMapperState) -> APIsImputOutputMapperState:
    """Execute main processing for APIsImputOutputMapper"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "APIsImputOutputMapper".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de APIsImputOutputMapper concluído com sucesso")],
        ...
        return {
            **state,
            "error_state": f"Error in APIsImputOutputMapper node: {str(e)}"
        }
...
# Add nodes
workflow_graph.add_node("main_processing", execute_apisimputoutputmapper_node)
...
def run_apisimputoutputmapper_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the APIsImputOutputMapper workflow via LangGraph"""
    initial_state = {
        "messages": messages,
