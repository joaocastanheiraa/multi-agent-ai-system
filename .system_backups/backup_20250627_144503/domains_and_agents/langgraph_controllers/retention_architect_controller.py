...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are retention_architect, a specialized AI agent.
...
- **knowledge_base**: Tool knowledge_base para retention_architect (✅ ATIVA)
- **text_analysis**: Tool text_analysis para retention_architect (✅ ATIVA)
- **web_search**: Tool web_search para retention_architect (✅ ATIVA)
...
- **transition_specialist**: Sub-agente especializado
- **immersion_architect**: Sub-agente especializado
- **journey_cartographer**: Sub-agente especializado
- **rhythm_programmer**: Sub-agente especializado
- **tension_engineer**: Sub-agente especializado
...
class FunctionalRetentionArchitectController:
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é RETENTION_ARCHITECT, um assistente AI especializado.
...
def execute_retention_architect_node(state: retentionarchitectState) -> retentionarchitectState:
    """Execute main processing for retention architect"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "retention architect".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de retention architect concluído com sucesso")],
                'agent_name': "retention architect",
                'current_step': 'completed'
            }
        ...
        return {
            **state,
            "error_state": f"Error in retention architect node: {str(e)}"
        }
...
# Add nodes
workflow_graph.add_node("main_processing", execute_retention_architect_node)
...
def run_retention_architect_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the retention architect workflow via LangGraph"""
    initial_state = {
        "messages": messages,
