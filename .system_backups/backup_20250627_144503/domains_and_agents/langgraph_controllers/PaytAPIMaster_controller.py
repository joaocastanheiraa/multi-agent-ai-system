...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are PaytAPIMaster, a specialized AI agent.
...
#!/usr/bin/env python3
"""
🤖 PAYTAPIMASTER - CONTROLLER FUNCIONAL
...
class FunctionalPaytapimasterController:
    """Controller funcional do paytapimaster"""
    
    def __init__(self):
        self.agent_name = "paytapimaster"
        self.domain = "apis"
        self.setup_llm()
        self.load_prompt()
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é PAYTAPIMASTER, um especialista em integração de APIs.
...
# Instância global
functional_paytapimaster = FunctionalPaytapimasterController()

def run_paytapimaster(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Função principal de execução"""
    return functional_paytapimaster.execute(messages)
...
def execute_paytapimaster_node(state: PaytAPIMasterState) -> PaytAPIMasterState:
    """Execute main processing for PaytAPIMaster"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "PaytAPIMaster".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de PaytAPIMaster concluído com sucesso")],
                'agent_name': "PaytAPIMaster",
                'current_step': 'completed'
            }
        ...
        return {
            **state,
            "error_state": f"Error in PaytAPIMaster node: {str(e)}"
        }

# Create the StateGraph
workflow_graph = StateGraph(PaytAPIMasterState)

# Add nodes
workflow_graph.add_node("main_processing", execute_paytapimaster_node)
...
PaytAPIMaster_graph = workflow_graph.compile()

def run_paytapimaster_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the PaytAPIMaster workflow via LangGraph"""
    initial_state = {
        "messages": messages,
    ...
    final_state = PaytAPIMaster_graph.invoke(initial_state)
    return final_state


if __name__ == "__main__":
    # Teste do controller
    print(f"🤖 TESTANDO {functional_paytapimaster.agent_name.upper()} FUNCIONAL")
    print("=" * 50)
    
    test_messages = [HumanMessage(content="Teste de funcionamento do agente paytapimaster")]
    result = run_paytapimaster(test_messages)
