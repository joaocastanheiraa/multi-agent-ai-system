...
# ================================================================================
ENHANCED_SYSTEM_PROMPT = """You are pain_detector, a specialized AI agent.
...
- **emotion_analyzer**: Tool emotion_analyzer para pain_detector (✅ ATIVA)
- **empathy_mapper**: Tool empathy_mapper para pain_detector (✅ ATIVA)
- **knowledge_base**: Tool knowledge_base para pain_detector (✅ ATIVA)
- **pain_classifier**: Tool pain_classifier para pain_detector (✅ ATIVA)
- **text_analysis**: Tool text_analysis para pain_detector (✅ ATIVA)
- **web_search**: Tool web_search para pain_detector (✅ ATIVA)
...
- **context-cartographer**: Sub-agente especializado
- **symptom-translator**: Sub-agente especializado
- **impact-prioritizer**: Sub-agente especializado
- **consequence-amplifier**: Sub-agente especializado
- **digital-ethnographer**: Sub-agente especializado
...
class FunctionalPainDetectorController:
    ...
    def load_prompt(self):
        ...
        self.system_prompt = """Você é PAIN_DETECTOR, um especialista em copywriting e persuasão.
...
def execute_pain_detector_node(state: paindetectorState) -> paindetectorState:
    """Execute main processing for pain detector"""
    try:
        current_messages = state.get("messages", [])
        
        # Usar o controller funcional existente
        # Procurar pela função run_* correspondente
        run_function_name = "run_" + "pain detector".lower().replace(" ", "_").replace("|", "_").replace("-", "_")
        ...
        if run_function_name in globals():
            ...
            result = {
                'success': True,
                'messages': current_messages + [AIMessage(content=f"Processamento de pain detector concluído com sucesso")],
                'agent_name': "pain detector",
                'current_step': 'completed'
            }
        ...
        return {
            **state,
            "error_state": f"Error in pain detector node: {str(e)}"
        }
...
# Add nodes
workflow_graph.add_node("main_processing", execute_pain_detector_node)
...
def run_pain_detector_langgraph(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the pain detector workflow via LangGraph"""
    initial_state = {
        "messages": messages,
