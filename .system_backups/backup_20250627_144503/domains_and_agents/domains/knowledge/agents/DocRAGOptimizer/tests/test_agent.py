#!/usr/bin/env python3
"""
🧪 TESTE OTIMIZADO - DOCRAGOPTIMIZER
...
try:
    from ..DocRAGOptimizer_controller import run_DocRAGOptimizer_optimized
except ImportError:
    ...
    try:
        ...
        run_DocRAGOptimizer_optimized = None

class TestOptimizedDocRAGOptimizer:
    """🧪 Testes para agente otimizado"""
    
    @pytest.mark.asyncio
    async def test_basic_execution(self):
        """Teste básico de execução"""
        if run_DocRAGOptimizer_optimized is None:
            pytest.skip("Controller otimizado não disponível")
            
        request = "Teste básico do agente otimizado"
        result = await run_DocRAGOptimizer_optimized(request)
    ...
    async def test_context_handling(self):
        ...
        result = await run_DocRAGOptimizer_optimized(request, context)
...
if __name__ == "__main__":
    async def run_tests():
        ...
        print(f"🧪 EXECUTANDO TESTES PARA DOCRAGOPTIMIZER")
        print("=" * 50)
        
        test_instance = TestOptimizedDocRAGOptimizer()
