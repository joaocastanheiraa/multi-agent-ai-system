#!/usr/bin/env python3
"""
🧪 TESTE OTIMIZADO - PERFECTPAYAPIMASTER
...
try:
    from ..PerfectpayAPIMaster_controller import run_PerfectpayAPIMaster_optimized
except ImportError:
    ...
    try:
        ...
        run_PerfectpayAPIMaster_optimized = None

class TestOptimizedPerfectpayAPIMaster:
    """🧪 Testes para agente otimizado"""
    
    @pytest.mark.asyncio
    async def test_basic_execution(self):
        """Teste básico de execução"""
        if run_PerfectpayAPIMaster_optimized is None:
            pytest.skip("Controller otimizado não disponível")
            
        request = "Teste básico do agente otimizado"
        result = await run_PerfectpayAPIMaster_optimized(request)
    ...
    async def test_context_handling(self):
        ...
        result = await run_PerfectpayAPIMaster_optimized(request, context)
...
if __name__ == "__main__":
    async def run_tests():
        ...
        print(f"🧪 EXECUTANDO TESTES PARA PERFECTPAYAPIMASTER")
        print("=" * 50)
        
        test_instance = TestOptimizedPerfectpayAPIMaster()
