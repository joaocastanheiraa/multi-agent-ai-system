#!/usr/bin/env python3
"""
🧪 TESTE OTIMIZADO - HOTMARTAPIMASTER
...
try:
    from ..HotmartAPIMaster_controller import run_HotmartAPIMaster_optimized
except ImportError:
    ...
    try:
        ...
        run_HotmartAPIMaster_optimized = None

class TestOptimizedHotmartAPIMaster:
    """🧪 Testes para agente otimizado"""
    
    @pytest.mark.asyncio
    async def test_basic_execution(self):
        """Teste básico de execução"""
        if run_HotmartAPIMaster_optimized is None:
            pytest.skip("Controller otimizado não disponível")
            
        request = "Teste básico do agente otimizado"
        result = await run_HotmartAPIMaster_optimized(request)
    ...
    async def test_context_handling(self):
        ...
        result = await run_HotmartAPIMaster_optimized(request, context)
...
if __name__ == "__main__":
    async def run_tests():
        ...
        print(f"🧪 EXECUTANDO TESTES PARA HOTMARTAPIMASTER")
        print("=" * 50)
        
        test_instance = TestOptimizedHotmartAPIMaster()
