#!/usr/bin/env python3
"""
🤖 OUTPUT_MAPPER - SUB-AGENTE OTIMIZADO
...
class OutputMapperSubAgent:
    """🤖 Sub-agente otimizado com funcionalidades LangChain avançadas"""
    
    def __init__(self):
        self.agent_name = "output_mapper_subagent_optimized"
        self.domain = "apis"
...
# Instância global
output_mapper_subagent = OutputMapperSubAgent()

async def run_output_mapper_subagent(request: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    """🤖 Função principal do sub-agente otimizado"""
    return await output_mapper_subagent.execute_subagent(request, context)

# Função de compatibilidade
def output_mapper_agent(request: str) -> str:
    """🔄 Função de compatibilidade"""
    try:
        result = asyncio.run(run_output_mapper_subagent(request))
...
if __name__ == "__main__":
    async def test_subagent():
        print(f"🧪 TESTANDO {output_mapper_subagent.agent_name}")
        result = await run_output_mapper_subagent("Teste do sub-agente otimizado")
