...
try:
    from ..PaytAPIMaster_controller import run_PaytAPIMaster_optimized
except ImportError:
    ...
    try:
        ...
        run_PaytAPIMaster_optimized = None

class TestOptimizedPaytAPIMaster:
    """🧪 Testes para agente otimizado"""
    
    @pytest.mark.asyncio
    async def test_basic_execution(self):
        """Teste básico de execução"""
        if run_PaytAPIMaster_optimized is None:
            pytest.skip("Controller otimizado não disponível")
            
        request = "Teste básico do agente otimizado"
        result = await run_PaytAPIMaster_optimized(request)
    ...
    async def test_context_handling(self):
        ...
        result = await run_PaytAPIMaster_optimized(request, context)
...
if __name__ == "__main__":
    async def run_tests():
        ...
        print(f"🧪 EXECUTANDO TESTES PARA PAYTAPIMASTER")
        print("=" * 50)
        
        test_instance = TestOptimizedPaytAPIMaster()
