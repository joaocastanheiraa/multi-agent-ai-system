#!/usr/bin/env python3
"""
LangGraph Controller for HotmartAPIMaster
...
def execute_step_1(state: HotmartapimasterState) -> HotmartapimasterState:
...
def execute_step_2(state: HotmartapimasterState) -> HotmartapimasterState:
...
def execute_step_3(state: HotmartapimasterState) -> HotmartapimasterState:
...
def run_HotmartAPIMaster(messages: List[BaseMessage]) -> Dict[str, Any]:
    """Run the HotmartAPIMaster workflow"""
    initial_state = {
        "messages": messages,
        "current_step": "step_1",
...
if __name__ == "__main__":
    ...
    test_messages = [HumanMessage(content="Test message for HotmartAPIMaster")]
    result = run_HotmartAPIMaster(test_messages)
    
    print(f"Workflow completed for HotmartAPIMaster")
