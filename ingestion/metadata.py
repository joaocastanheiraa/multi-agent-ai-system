...
def extract_metadata_from_path(file_path, base_dir):
    ...
    subdomain = (
        os.path.dirname(rel_path).replace(parts[0] + os.sep, "")
        if len(parts) > 1
        else None
    )
    ...
    if agent_principal == "NEUROHOOK-ULTRA" and subdomain:
        ...
        matching_subdomains = [
            sd for sd in NEUROHOOK_SUBDOMAIN_TO_SUBAGENT if subdomain.startswith(sd)
        ]
        ...
        matching_subdomains = [
            sd for sd in RETENTION_SUBDOMAIN_TO_SUBAGENT if subdomain.startswith(sd)
        ]
        if matching_subdomains:
            best_match = max(matching_subdomains, key=len)
        ...
        matching_subdomains = [
            sd for sd in PAIN_SUBDOMAIN_TO_SUBAGENT if subdomain.startswith(sd)
        ]
        if matching_subdomains:
            best_match = max(matching_subdomains, key=len)
