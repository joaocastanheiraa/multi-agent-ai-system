...
from ingestion.readers import read_markdown_file, find_files, get_relative_path
...
def process_file(file_path, base_dir, max_tokens=500):
    ...
    for i, chunk_text in enumerate(chunks_text):
        try:
            ...
            logger.error(f"Erro ao processar chunk {i} do arquivo {file_path}: {str(e)}")
...
def run_ingestion(knowledge_dir, extensions=['.md'], max_tokens=500):
    ...
    if all_chunks:
        ...
        logger.info(f"Inserção concluída. {len(inserted_ids)} chunks inseridos com sucesso.")
    else:
